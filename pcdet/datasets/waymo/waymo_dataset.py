...
class WaymoDataset(DatasetTemplate):
    ...
    def create_groundtruth_database(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10, processed_data_tag=None):
        def get_gpu_memory_usage():
            if torch.cuda.is_available():
                ...
                for i in range(gpu_count):
                    ...
                    gpu_memory_usage[f'GPU {i}'] = {'Allocated (GB)': allocated, 'Reserved (GB)': reserved}
                return gpu_memory_usage
            else:
                return "No GPU available."

        # Allocate memory to keep GPU engaged to avoid HPC terminating rules. Delete this part if you own the machine.
        dummy_tensor = torch.randn(1024**3 * 10 // 4, device='cuda')  # 10 GB dummy tensor
        ...
        for k in range(0, len(infos), sampled_interval):
            dummy_tensor = dummy_tensor*0.99
            if k%100==0:
                gpu_usage = get_gpu_memory_usage()
                self.logger.info(str(k) + ' ' + str(gpu_usage))
